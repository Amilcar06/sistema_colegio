...
public class EstudianteController {
        ...
        @Operation(summary = "Listar estudiantes", description = "Lista todos los estudiantes registrados. Puede filtrarse por estado (activo/inactivo).")
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Estudiante encontrado", content = @Content(schema = @Schema(implementation = EstudianteResponseDTO.class))),
                        @ApiResponse(responseCode = "403", description = "Acceso denegado"),
                        @ApiResponse(responseCode = "404", description = "Estudiante no encontrado")
        })
        @PreAuthorize("hasAnyRole('ROLE_DIRECTOR', 'ROLE_SECRETARIA', 'ROLE_ESTUDIANTE')")
        @GetMapping("/{id}")
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Perfil del estudiante obtenido correctamente", content = @Content(schema = @Schema(implementation = EstudianteResponseDTO.class))),
                        @ApiResponse(responseCode = "404", description = "Estudiante no encontrado")
        })
        ...
        @Operation(summary = "Listar gestiones del estudiante", description = "Devuelve el historial de gestiones académicas donde el estudiante ha estado inscrito.")
        @ApiResponse(responseCode = "200", description = "Listado de gestiones obtenido")
        @PreAuthorize("hasAnyRole('ROLE_ESTUDIANTE', 'ROLE_DIRECTOR', 'ROLE_SECRETARIA')")
        @GetMapping("/{id}/gestiones")
        public ResponseEntity<List<com.unidadeducativa.academia.gestion.dto.GestionResponseDTO>> listarGestiones(
                        @Parameter(description = "ID del estudiante") @PathVariable Long id) {
                return ResponseEntity.ok(estudianteService.listarGestionesEstudiante(id));
        }
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Estudiante actualizado exitosamente", content = @Content(schema = @Schema(implementation = EstudianteResponseDTO.class))),
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Perfil actualizado correctamente", content = @Content(schema = @Schema(implementation = EstudianteResponseDTO.class))),
                        @ApiResponse(responseCode = "400", description = "Datos inválidos"),
